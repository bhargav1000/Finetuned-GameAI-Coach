...
export class Agent {
    ...
    updateQ(prevState, action, reward, nextState) {
        // Skip Q-table updates in demo mode to preserve learning state
        if (this.demoMode) {
            return;
        }
     ...
     */
    saveQ() {
        // Skip saving in demo mode to preserve original learning state
        if (this.demoMode) {
            console.log(`Q-table for ${this.name} save skipped (demo mode - preserving learning state).`);
            return;
        }
