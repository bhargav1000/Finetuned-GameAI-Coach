...
class DemoPlayScene extends Phaser.Scene {
    ...
    init() {
        ...
        // Initialize action history tracking for enhanced AI context
        this.heroActionHistory = [];
        this.knightActionHistory = [];
        this.combatEvents = [];
        this.maxHistoryLength = 10; // Keep last 10 actions
        this.lastDamageEvent = null;
        this.combatMomentum = 'neutral'; // 'hero_advantage', 'knight_advantage', 'neutral'
    ...
    async emitNewGameEvent() {
        try {
            ...
            const response = await fetch('http://localhost:8766/game_event', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                ...
                console.warn('Failed to send new game event - AI assistant server may not be running');
            }
        } catch (error) {
            console.warn('Could not send new game event to AI server:', error.message);
    ...
    async requestAISuggestion() {
        ...
        try {
            const response = await fetch('http://localhost:8766/ai_suggestion', {
                ...
                body: JSON.stringify({ game_state: gameState, timestamp: Date.now() })
            });
            if (response.ok) {
                const data = await response.json();
                this.updateDemoInterface(data);
            } else {
                console.warn('AI suggestion request failed.');
            }
        } catch (error) {
            console.warn('Error requesting AI suggestion:', error.message);
        }
    }

    getCurrentGameStateString() {
        if (!this.hero || !this.purpleKnight) return "";
        
        const distance = Phaser.Math.Distance.Between(this.hero.x, this.hero.y, this.purpleKnight.x, this.purpleKnight.y);
        const distanceCategory = distance < 100 ? 'close' : (distance < 250 ? 'medium' : 'far');
        const gamePhase = this.time.now < 30000 ? 'early_game' : (this.time.now < 90000 ? 'mid_game' : 'critical');
        
        // Character status analysis
        const heroStatus = this.hero.isAttacking ? 'attacking' : 
                          (this.hero.isBlocking ? 'blocking' : 
                          (this.hero.isTakingDamage ? 'taking_damage' : 'idle'));
        
        const knightStatus = this.purpleKnight.isAttacking ? 'attacking' : 
                            (this.purpleKnight.isBlocking ? 'blocking' : 
                            (this.purpleKnight.isTakingDamage ? 'taking_damage' : 'idle'));
        
        // Stamina analysis
        const heroStaminaLevel = this.hero.stamina / this.hero.maxStamina;
        const knightStaminaLevel = this.purpleKnight.stamina / this.purpleKnight.maxStamina;
        const heroStaminaStatus = heroStaminaLevel > 0.7 ? 'high' : (heroStaminaLevel > 0.3 ? 'medium' : 'low');
        const knightStaminaStatus = knightStaminaLevel > 0.7 ? 'high' : (knightStaminaLevel > 0.3 ? 'medium' : 'low');
        
        // Recent action context (last 3 actions for brevity)
        const recentHeroActions = this.heroActionHistory?.slice(-3).map(a => a.action).join(', ') || 'none';
        const recentKnightActions = this.knightActionHistory?.slice(-3).map(a => a.action).join(', ') || 'none';
        
        // Combat momentum and recent events
        const recentDamage = this.lastDamageEvent ? 
            `${this.lastDamageEvent.victim} took damage from ${this.lastDamageEvent.attacker} (${this.time.now - this.lastDamageEvent.timestamp < 2000 ? 'just now' : 'recently'})` : 
            'no recent damage';
        
        // Health analysis for tactical context
        const heroHealthLevel = this.hero.health / this.hero.maxHealth;
        const knightHealthLevel = this.purpleKnight.health / this.purpleKnight.maxHealth;
        const healthAdvantage = heroHealthLevel > knightHealthLevel + 0.2 ? 'hero_advantage' : 
                               (knightHealthLevel > heroHealthLevel + 0.2 ? 'knight_advantage' : 'even');
        
        // Construct enhanced game state
        const gameState = `
            Hero Health: ${Math.round(heroHealthLevel * 100)}% (${heroHealthLevel > 0.6 ? 'healthy' : heroHealthLevel > 0.3 ? 'wounded' : 'critical'}), 
            Hero Stamina: ${Math.round(heroStaminaLevel * 100)}% (${heroStaminaStatus}), 
            Hero Status: ${heroStatus}, 
            Knight Health: ${Math.round(knightHealthLevel * 100)}% (${knightHealthLevel > 0.6 ? 'healthy' : knightHealthLevel > 0.3 ? 'wounded' : 'critical'}), 
            Knight Stamina: ${Math.round(knightStaminaLevel * 100)}% (${knightStaminaStatus}), 
            Knight Status: ${knightStatus}, 
            Distance: ${distanceCategory} (${Math.round(distance)}px), 
            Game Phase: ${gamePhase}, 
            Health Advantage: ${healthAdvantage}, 
            Combat Momentum: ${this.combatMomentum}, 
            Recent Hero Actions: [${recentHeroActions}], 
            Recent Knight Actions: [${recentKnightActions}], 
            Recent Combat: ${recentDamage}
        `;
        
        return gameState.trim().replace(/\s+/g, ' ');
    }

    // =================================================================
    //  ACTION TRACKING FUNCTIONS FOR ENHANCED AI CONTEXT
    // =================================================================
    
    trackHeroAction(action) {
        if (!this.heroActionHistory) this.heroActionHistory = [];
        
        const timestamp = this.time.now;
        this.heroActionHistory.push({
            action: action,
            timestamp: timestamp
        });
        
        // Keep only recent actions
        if (this.heroActionHistory.length > this.maxHistoryLength) {
            this.heroActionHistory.shift();
        }
        
        // Update combat momentum
        this.updateCombatMomentum('hero', action);
        
        console.log(`🎯 Hero Action Tracked: ${action}`);
    }
    
    trackKnightAction(action) {
        if (!this.knightActionHistory) this.knightActionHistory = [];
        
        const timestamp = this.time.now;
        this.knightActionHistory.push({
            action: action,
            timestamp: timestamp
        });
        
        // Keep only recent actions
        if (this.knightActionHistory.length > this.maxHistoryLength) {
            this.knightActionHistory.shift();
        }
        
        // Update combat momentum
        this.updateCombatMomentum('knight', action);
        
        console.log(`⚔️ Knight Action Tracked: ${action}`);
    }
    
    trackDamageEvent(attacker, victim, damage) {
        ...
        // Add to combat events
        if (!this.combatEvents) this.combatEvents = [];
        this.combatEvents.push(this.lastDamageEvent);
        
        // Keep only recent events
        if (this.combatEvents.length > 5) {
            this.combatEvents.shift();
        }
        
        console.log(`💥 Damage Event: ${attacker} -> ${victim} (${damage} damage)`);
    }
    
    updateCombatMomentum(actor, action) {
        // Simple momentum tracking based on aggressive vs defensive actions
        const aggressiveActions = ['attack', 'melee', 'melee2', 'special1', 'heavy'];
        const defensiveActions = ['block', 'dodge', 'roll'];
        
        if (aggressiveActions.includes(action)) {
            if (actor === 'hero') {
                this.combatMomentum = 'hero_advantage';
            } else {
                this.combatMomentum = 'knight_advantage';
            }
        } else if (defensiveActions.includes(action)) {
            // Defensive actions tend toward neutral
            this.combatMomentum = 'neutral';
        }
        
        // Momentum decays over time
        if (this.time.now - this.lastMomentumUpdate > 5000) {
            this.combatMomentum = 'neutral';
        }
        this.lastMomentumUpdate = this.time.now;
    }

    updateDemoInterface(responseData) {
        const { suggestion, model_used, confidence } = responseData;
        
        console.log("AI Suggestion received:", responseData);
        
        // Clean up the suggestion text to extract only the actual advice
        let cleanedSuggestion = suggestion || "";
        
        // Extract just the actual tactical advice, removing all the prompt and context
        // Since we've modified the AI server to include "What's the best tactical move right now?"
        // as a separator, we can simply split on that phrase
        if (cleanedSuggestion.includes("What's the best tactical move right now?")) {
            cleanedSuggestion = cleanedSuggestion.split("What's the best tactical move right now?")[1].trim();
        } else {
            // Fallback patterns if the separator isn't found
            const patterns = [
                // If there's a clear tactical suggestion with an emoji at the start
                /[🔥⚔️🛡️🏃‍♂️🤺💪⚡🎯🔄][^.!?]*[.!?]/,
                
                // Remove any system prompt text and game state
                /Current battle situation:.*?((?:[A-Z][^.!?]*[.!?])+)$/s,
                
                // Last resort - take the last sentence if it's short and looks like advice
                /([^.!?]{5,60}[.!?])$/
            ];
            
            for (const pattern of patterns) {
                const match = cleanedSuggestion.match(pattern);
                if (match && match[1]) {
                    const extracted = match[1].trim();
                    if (extracted && 
                        !extracted.includes("Focus on:") && 
                        !extracted.includes("Hero Health:") &&
                        !extracted.includes("Game Phase:")) {
                        cleanedSuggestion = extracted;
                        break;
                    }
                }
            }
            
            // If we still have a long text, just take the last sentence
            if (cleanedSuggestion.length > 100) {
                const sentences = cleanedSuggestion.split(/[.!?]+/);
                const lastSentence = sentences[sentences.length - 2]; // -2 because split creates an empty element at the end
                if (lastSentence && lastSentence.trim().length > 5) {
                    cleanedSuggestion = lastSentence.trim();
                }
            }
        }
        
        // Use the window.demoInterface functions defined in demo.html
        if (window.demoInterface) {
            // Update the suggestion text with the cleaned version
            window.demoInterface.updateAISuggestion(cleanedSuggestion, confidence);
            
            // Update the model status
            window.demoInterface.setModelStatus(model_used || "Phi-3.5");
            
            // Update game state
            const gameState = {
                heroHealth: this.hero.health / this.hero.maxHealth,
                heroStamina: this.hero.stamina / this.hero.maxStamina,
                knightHealth: this.purpleKnight.health / this.purpleKnight.maxHealth,
                knightStamina: this.purpleKnight.stamina / this.purpleKnight.maxStamina,
                distance: Phaser.Math.Distance.Between(this.hero.x, this.hero.y, this.purpleKnight.x, this.purpleKnight.y) < 100 ? 'close' : 
                         (Phaser.Math.Distance.Between(this.hero.x, this.hero.y, this.purpleKnight.x, this.purpleKnight.y) < 250 ? 'medium' : 'far'),
                phase: this.time.now < 30000 ? 'Early Game' : (this.time.now < 90000 ? 'Mid Game' : 'Critical Phase')
            };
            window.demoInterface.updateGameState(gameState);
        } else {
            console.warn("Demo interface not found! AI suggestions won't be displayed.");
        }
    }
    ...
    create() {
        ...
        this.keys.shift = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.SHIFT); // Block
        this.keys.ctrl = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.CTRL); // Dodge/Roll
        this.keys.x = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.X); // Special Attack
        this.keys.c = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.C); // Heavy Attack
        this.keys.p = this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.P); // Screenshot key

        // Debug keys (keeping for compatibility)
        ...
        this.controllerType = 'keyboard'; // 'keyboard' or 'xbox'
        
        // Initialize controller detection
        this.detectController();
        
        // Create control display in lower right corner
        ...
        // Note: The HTML already has an onclick handler that calls requestSuggestion(),
        // which in turn calls this.requestAISuggestion() via window.demoGame
    }
        ...
        if (navigator.getGamepads) {
            ...
            for (let i = 0; i < gamepads.length; i++) {
                if (gamepads[i]) {
                    ...
                    console.log('🎮 Xbox controller detected:', gamepads[i].id);
    ...
    createControlDisplay() {
        // Create control display container in lower right corner
        this.controlDisplay = this.add.container(this.game.config.width - 300, this.game.config.height - 200);
        this.controlDisplay.setScrollFactor(0);
        this.controlDisplay.setDepth(150);
        
        // Background panel
        this.controlBg = this.add.graphics();
        this.controlBg.fillStyle(0x000000, 0.8);
        this.controlBg.fillRoundedRect(0, 0, 280, 180, 10);
        this.controlBg.lineStyle(2, 0xffffff, 0.5);
        this.controlBg.strokeRoundedRect(0, 0, 280, 180, 10);
        this.controlDisplay.add(this.controlBg);
        
        // Title
        this.controlTitle = this.add.text(140, 15, 'CONTROLS', {
            fontSize: '16px',
            fill: '#ffffff',
            fontStyle: 'bold'
        }).setOrigin(0.5);
        this.controlDisplay.add(this.controlTitle);
        
        // Control text (will be updated based on input method)
        this.controlText = this.add.text(10, 40, '', {
            fontSize: '12px',
            fill: '#ffffff',
            wordWrap: { width: 260 }
        });
        this.controlDisplay.add(this.controlText);
        
        // Update with initial keyboard controls
        this.updateControlDisplay();
    }

    updateControlDisplay() {
        if (!this.controlText) return;
...
• C: Heavy Attack`;
        ...
        this.controlText.setText(controls);
        
        // Update title color based on controller type
        if (this.controlTitle) {
            this.controlTitle.setColor(this.controllerType === 'xbox' ? '#00ff00' : '#ffff00');
        }
    }
    ...
    handleUserInput() {
        ...
        if (this.controllerType === 'xbox' && this.gamepad) {
            inputAction = this.handleXboxInput();
        } else {
            inputAction = this.handleKeyboardInput();
        }
        
        if (inputAction) {
            this.executeHeroAction(inputAction);
        }
    }
    ...
    handleKeyboardInput() {
        ...
        if (keys.w.isDown || keys.up.isDown) moveY = -1;
        if (keys.s.isDown || keys.down.isDown) moveY = 1;
        if (keys.a.isDown || keys.left.isDown) moveX = -1;
        if (keys.d.isDown || keys.right.isDown) moveX = 1;
        ...
        if (moveX !== 0 || moveY !== 0) {
            ...
            return null; // Movement handled directly
        } else {
            // Stop movement
        ...
        if (keys.x.isDown) {
            return 'attack_special1';
        }
        if (keys.c.isDown) {
            return 'attack_melee2';
        }
    ...
    handleXboxInput() {
        ...
        if (Math.abs(leftStickX) > deadzone || Math.abs(leftStickY) > deadzone) {
            ...
            // Set velocity
            const speed = 5;
            this.hero.setVelocity(leftStickX * speed, leftStickY * speed);
            this.hero.anims.play(`walk-${this.facing}`, true);
            return null; // Movement handled directly
        } else {
            // Stop movement
    ...
    applyDamage = (target, damage) => {
        ...
        // Track damage event for enhanced AI context
        const attackerName = target === this.hero ? 'knight' : 'hero';
        const victimName = target === this.hero ? 'hero' : 'knight';
        this.trackDamageEvent(attackerName, victimName, damage);
    ...
    executeHeroAction(action) {
        ...
        // Track hero action for enhanced AI context
        this.trackHeroAction(action);
    ...
    executeKnightAction(action) {
        ...
        // Track knight action for enhanced AI context
        this.trackKnightAction(action);
    ...
    update(time, delta) {
        ...
        // --- Auto AI Suggestions ---
        // Check if auto suggestions are enabled via the demo interface
        if (window.demoInterface && window.demoInterface.autoSuggestionsEnabled()) {
            if (!this.lastSuggestionTime || time - this.lastSuggestionTime > 5000) { // Every 5 seconds
                this.requestAISuggestion();
                this.lastSuggestionTime = time;
            }
        }
        ...
        if (!this.isDeathSequenceActive) {
            // Handle hero idle animation (user-controlled, so only when not moving)
            if (this.hero.body.velocity.x === 0 && this.hero.body.velocity.y === 0 && !this.hero.isAttacking && !this.hero.isBlocking) {
                this.hero.anims.play(`idle-${this.facing}`, true);
            }
            
            // Handle purple knight tracking (AI-controlled)
            this.handleIdleAndTracking(this.purpleKnight, this.hero);
        }
    ...
    takeScreenshot() {
        ...
        fetch('http://localhost:8766/game_state_snapshot', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
window.demoGame = new Phaser.Game(config); 